...
class fewshotI2P(baseI2P):
    def __init__(self, cfg):
        ...
        # add mlp layers for eq. (12), (14), (15) implementation
        self.mlp_a = nn.Linear(128, 128)
        self.mlp_b = nn.Linear(128, 128)
    ...
    def forward(self, data_dict):
        ...
        img_fea_mean_array_n = img_fea_mean_array.unsqueeze(0)
        pts_fea_mean_array_n = pts_fea_mean_array.unsqueeze(0)
        img_fea_mean_array_n = torch.nn.functional.normalize(img_fea_mean_array_n, p=2, dim=-1)
        pts_fea_mean_array_n = torch.nn.functional.normalize(pts_fea_mean_array_n, p=2, dim=-1)

        # TODO eq.(12) implementation
        pass

        mat_primitive = pairwiseL2Dist(img_fea_mean_array_n, pts_fea_mean_array_n)
        # Sinkhorn layer configuration
        ...
        # eq.(14) and (15) implementation
        # this module can be stacked
        '''
            img_fea_mean_array:  torch.Size([20, 128])
            pts_fea_mean_array:  torch.Size([57, 128])
            mat_primitive:  torch.Size([20, 57])
        '''
        alpha = 0.75
        alpha = 0.85
        # alpha = 0.90
        ...
        img_fea_mean_array_new = self.mlp_a(alpha*img_fea_mean_array + \
            (1-alpha)*torch.matmul(mat_primitive, pts_fea_mean_array))
        pts_fea_mean_array_new = self.mlp_b(alpha*pts_fea_mean_array + \
            (1-alpha)*torch.matmul(mat_primitive.t(), img_fea_mean_array))
        # img_fea_mean_array_new = alpha*img_fea_mean_array + \
        #     (1-alpha)*torch.matmul(mat_primitive, pts_fea_mean_array)#/11.313
        # pts_fea_mean_array_new = alpha*pts_fea_mean_array + \
        #     (1-alpha)*torch.matmul(mat_primitive.t(), img_fea_mean_array)#/11.313
            
        # 3.3 update image and point cloud features
        #     with  img_fea_delt_sels,       pts_fea_delt_sels
        #     and   img_fea_mean_array_new,  pts_fea_mean_array_new
        #     img_feats_f:  torch.Size([1, 128, 480, 640])
        c, h, w = img_feats_f.size(1), img_feats_f.size(2), img_feats_f.size(3)
        df = torch.zeros((h,w,c)).cuda() # (480,640,128)
        for i in range(num_img_seg):
            mask = img_seg_mask[i]
            df[mask] = img_fea_mean_array_new[i] - img_fea_mean_array[i]
        df = df.transpose(2,0).transpose(2,1).unsqueeze(0) 
        img_feats_f_new = img_feats_f+df
        img_feats_f = alpha*img_feats_f + (1-alpha)*img_feats_f_new
        for i in range(num_pts_seg):
            dx = pts_fea_delt_sels[i]
            x  = pts_fea_mean_array_new[i]
            pts_fea_delt_sels[i] = x+dx
        pcd_feats_f_new = torch.concat(pts_fea_delt_sels)
        pcd_feats_f = alpha*pcd_feats_f + (1-alpha)*pcd_feats_f_new
