...
# if need visualize latent features
from .vis_feature import latent_feature_vis
...
class TopI2PPlus(baseI2P):
    ...
    def construct_dis_mat(self, a, b):
        '''
            a [n,c] tensor
            b [m,c] tensor
        '''
    ...
    def forward(self, data_dict):
        ...
        an_tim_2 = time.time()
        # print("==> mp-mining costs time: ", an_tim_2-an_tim_1)
        ...
        an_tim_2 = time.time()
        # print("==> he-adapting costs time: ", an_tim_2-an_tim_1)

        # note-1029 
        #   only for the feature visulization
        # img_feats_list = self.img_backbone(img_feats)
        # pcd_feats_list = self.pcd_backbone(pcd_feats, data_dict)
        # latent_feature_vis(
        #     img_feats_list[0], pcd_feats_list[0], img_feats_f, pcd_feats_f)
        ...
        if not self.training:
            ...
            if is_tr_prune == True:
                ...
                '''
                # pre: estimate pose prior with topological relations
                seg_2d_mask = data_dict["seg_2d_mask"].detach().bool() # [m,h,w]
                seg_3d_mask = data_dict["seg_3d_mask"].detach().bool() # [n,N]
                seg_2d_ct   = data_dict["seg_2d_ct"].detach() # [m,2]
                seg_3d_ct   = data_dict["seg_3d_ct"].detach() # [n,3]
                num_2d = seg_2d_mask.size(0)
                num_3d = seg_3d_mask.size(0)
                seg_2d_mask = seg_2d_mask.reshape((num_2d,-1))         # [m,hw]
                # 0.5 => 0.0005
                relax_mat_c = (mat_primitive>0.0005).int().detach()    # [m,n]
                
                paired_2d_ct = torch.zeros((0,2)).cuda()
                paired_3d_ct = torch.zeros((0,3)).cuda()
                for i in range(num_2d):
                    ...
                    for j in range(num_paired):
                        paired_2d_ct = torch.concat(
                            (paired_2d_ct, seg_2d_ct[i:i+1]), dim=0)
                        k = idx[j]
                        paired_3d_ct = torch.concat(
                            (paired_3d_ct, seg_3d_ct[k:k+1]), dim=0)
                
                paired_2d_ct = torch.tensor(paired_2d_ct)
                paired_3d_ct = torch.tensor(paired_3d_ct)
                ct_2d = paired_2d_ct.cpu().numpy()
                ct_3d = paired_3d_ct.cpu().numpy()
                k_mat = intrinsics.cpu().numpy()

                success, rvec, tvec, inliers = \
                    cv.solvePnPRansac(ct_3d, ct_2d, k_mat, None,
                ...
                T_opt = torch.from_numpy(T_opt).cuda().float()

                err = torch.norm(T_opt - T0)
                if err >= 0.075:
                    T_opt = T0
                    print("... not stable in tr-pnp :(")
                
                '''
                    new step 6.1 --- tr-prune --- 
                        match refinement with topological relationships  
                '''
                ...
                att_mat_r = mat_primitive # [m,n] this mat is row-normalized
                if corr_sc.size(0) >= 200:
                    # =================== (module added) =================== #
                    # part I: group correspondences with tr 
                    valid_2d_all_pre = (corr_sc*0).bool()
                    proj_corr_3d = render(corr_3d, intrinsics, T_opt) # [d,2] 
                    for i in range(num_2d):
                        _, idx = torch.where(relax_mat_c[i:i+1,:] > 0.8)
                        num_paired = int(idx.size(0))
                        if num_paired == 0:
                        ...
                        #   maximum likelihood estimation with homography assumption
                        delta = corr_3d_pix - corr_2d_pix #[d,2]

                        # criterion I  --- distance threshold 
                        delta_pp = torch.norm(delta, dim=1)
                        keep_idp = (delta_pp <= 15) 
                        valid_2d = valid_2d[keep_idp]

                        # summary pruned correspondences
                        valid_2d_all_pre[valid_2d] = True
                    # =================== (module added) =================== #
                    
                    # exchange coordinates from (v,u) to (u,v)
                    tmp = seg_2d_ct[:,0]*1
                    seg_2d_ct[:,0] = seg_2d_ct[:,1]
                    seg_2d_ct[:,1] = tmp

                    # generate 2d corr-to-seg distance mat
                    corr_mat_2d = self.construct_dis_mat(corr_2d, seg_2d_ct) # [d,m]
                    # generate 3d corr-to-seg distance mat
                    proj_seg_3d_ct = render(seg_3d_ct, intrinsics, T_opt) # [n,2]
                    proj_corr_3d   = render(corr_3d,   intrinsics, T_opt) # [d,2]
                    proj_seg_3d_ct = proj_seg_3d_ct.float()
                    proj_corr_3d   = proj_corr_3d.float()
                    corr_mat_3d = self.construct_dis_mat(proj_corr_3d, proj_seg_3d_ct) # [d,n]
                    # print("corr_mat_2d: ", corr_mat_2d)
                    # print("corr_mat_3d: ", corr_mat_3d)

                    # correspondence score re-estimation 
                    # matrix nomralization row and colum
                    hh = int(att_mat_r.size(0))
                    ww = int(att_mat_r.size(1))
                    cc = min(hh, ww)
                    att_mat_r = att_mat_r * 0.0
                    for i in range(cc):
                        att_mat_r[i,i] = 1.0
                    att_mat_2d3d_r = F.normalize(att_mat_r.t(), p=1, dim=0)
                    att_mat_2d3d_c = F.normalize(att_mat_r,     p=1, dim=0)
                    # reasoning from heterogeneous graph
                    corr_mat_3d_o = torch.matmul(corr_mat_2d, att_mat_2d3d_c) # [d,n]
                    corr_mat_2d_o = torch.matmul(corr_mat_3d, att_mat_2d3d_r) # [d,m]
                    # print("corr_mat_2d_o: ", corr_mat_2d_o)
                    # print("corr_mat_3d_o: ", corr_mat_3d_o)
                    # assert 1==-1
                    
                    # compute l2 distance
                    err_2d = torch.abs(torch.mean(corr_mat_2d - corr_mat_2d_o, dim=1))
                    err_3d = torch.abs(torch.mean(corr_mat_3d - corr_mat_3d_o, dim=1))
                    score_ = (err_2d + err_3d)/2.0
                    # compute l2 distance --- disable
                    # l2_2d = torch.sum(proj_corr_3d - corr_2d, dim=1)
                    # generate new correspondences using top-k
                    k_num = int(score_.size(0) * 0.85) # 0.75
                    _, indices = torch.topk(score_, k_num, largest=False)
                    valid_2d_all = (corr_sc*0).bool()
                    valid_2d_all[indices] = True

                    # using two criteria
                    valid_2d_all = valid_2d_all | valid_2d_all_pre
                    # using one criteria
                    # valid_2d_all = valid_2d_all_pre
                    ...
                    # print("err_2d: ", err_2d)
                    # print("err_3d: ", err_3d)
                    # print("max err_2d: ", torch.max(err_2d), " min err_2d: ", torch.min(err_2d))
                    # print("max err_3d: ", torch.max(err_3d), " min err_3d: ", torch.min(err_3d))
                
                tb = time.time()
                # print("==> hc-pruning cost time: ", tb-ta)
                # assert 1==-1

        torch.cuda.synchronize()
        duration = time.time() - start_time
        output_dict["duration"] = duration
        # print("=======> cost time (model inference): ", duration)
        return output_dict
