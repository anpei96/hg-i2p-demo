        ...
        if not self.training:
            ...
            if is_tr_prune == True:
                ...
                success, rvec, tvec, inliers = \
                    ...
                    iterationsCount=10000, reprojectionError=1, flags=cv.SOLVEPNP_P3P)
                ...
                # using T_opt instead of T0
                proj_corr_3d = render(corr_3d, intrinsics, T_opt) # [d,2] 
                # proj_corr_3d_c = render(corr_3d, intrinsics, T0) # [d,2] 
                ...
                for i in range(num_2d):
                    ...
                    # corr_3d_pix_c = proj_corr_3d_c[valid_2d]
                    ...
                    beta = 2.0
                    flex_th  = beta*torch.mean(delta_pp)
                    keep_idp = (delta_pp <= 8) # 8
                    valid_2d = valid_2d[keep_idp]
                    
                    # criterion II --- mae (prior+mle) method
                    # var_delta = delta.var(dim=0)
                    # avg_delta = delta.mean(dim=0)
                    # keep_2dx = \
                    #     (delta[:,0] >= avg_delta[0] - 1*var_delta[0]) & \
                    #     (delta[:,0] <= avg_delta[0] + 1*var_delta[0])
                    # keep_2dy = \
                    #     (delta[:,1] >= avg_delta[1] - 1*var_delta[1]) & \
                    #     (delta[:,1] <= avg_delta[1] + 1*var_delta[1])
                    # keep_idx = keep_2dx & keep_2dy
                    # keep_idx = keep_idx #| keep_idp
                    # valid_2d = valid_2d[keep_idx]
                    
                    '''
                        error analysis here
                            pixels shifts have the clusters 
                    '''
                    # print("block: ", i)
                    # # print(delta)
                    ...
                    # delta_c = corr_3d_pix_c - corr_2d_pix
                    # delta_c = delta_c.cpu().detach().numpy()
                    # fig, ax = plt.subplots()  # Create a figure and an axes.
                    # ax.scatter(delta[:,0], delta[:,1],     label="pd",color="green")
                    # ax.scatter(delta_c[:,0], delta_c[:,1], label="gt",color="blue")
