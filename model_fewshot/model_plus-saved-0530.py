...
class TopI2PPlus(baseI2P):
    ...
    def forward(self, data_dict):
        ...
        for i in range(num_2d):
            ...
            # msg = self.msg_decode_2d(torch.concat((msg_2d_a, msg_2d_b), dim=1))
            '''
                using tf
            '''
        ...
        for i in range(num_3d):
            ...
            # msg = self.msg_decode_3d(torch.concat((msg_3d_a, msg_3d_b), dim=1))
            '''
                using tf
            '''
        ...
        ta = time.time()
        tb = time.time()
        # print("a: ", output_dict["pcd_points_c"].size())
        # print("b: ", proj_pcd_points_c.size())
