...
class baseline_with_prune(baseI2P):
    ...
    def forward(self, data_dict):
        ...
        if not self.training:
            ...
            # in the debug stage, we uses the gt depth
            ...
            '''
                step x.1 check the computation results in step 1,2
                    in the gt depth, scale is 0.001 --- ok
            '''
            # if scale is unique (just in the ideal case)
            scale_colls = (v_colls_norm/w_colls_norm)/temp_d1 # [d,d,1]
            scale_colls = scale_colls[:,:,0]
            scale_a = 9.999e-4
            scale_b = 1.001e-3
            ...
            # print(scale_colls.size())
            # print(map_scale)
            # print("map mean: ", torch.mean(map_scale))
            # print(map_scale.size())
            # print(map_scale[valid_mask].size())
            # # print("num: ", torch.sum(valid_mask))
            # assert 1==-1
