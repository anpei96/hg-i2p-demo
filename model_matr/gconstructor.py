...
class GraphConstructorForI2PReg(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        pass
    def forward(self, correspondence, resolution,  name, descriptor, inlier_thresh):
        ...
        if descriptor=="predator":
            ...
            if (name == "3dmatch" and descriptor == "fcgf"):
                ...
                score[score<0.950]=0 # 0.995
                print("score: ", score)
                print(score.size())
                print("check vec: ", torch.sum(score, dim=1))
                print(torch.sum(score, dim=1).size())
                # assert 1==-1
            elif (descriptor == "spinnet" or descriptor == "d3feat") :
                # score[score<0.85]=0
                score[score<0.950]=0 # 0.95
