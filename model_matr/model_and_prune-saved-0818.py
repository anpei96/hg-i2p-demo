...
class baseline_with_prune(baseI2P):
    ...
    def forward(self, data_dict):
        ...
        if not self.training:
            ...
            '''
                scheme-6 our new try on fastmac
            '''
            ...
            if is_used_scheme_6:
                ...
                # remove the diag elements
                scales = \
                    torch.where(torch.isnan(scales), torch.full_like(scales, 0), scales)
                scales_1k = scales * 1e3

                # estimate sigma of distribution --- not good
                # idx_max = torch.argmax(visual_hist) # 0.0034
                # sca_avg = visual_bins[idx_max] #/ 1000.0
                # sca_up  = sca_avg + 0.0004*1e3
                # sca_dn  = sca_avg - 0.0004*1e3
                # sca_msk = (scales_1k >= sca_dn) & (scales_1k <= sca_up)
                # sca_val = scales_1k[sca_msk]
                # sca_sigma = torch.sqrt(torch.mean((sca_val - sca_avg)**2))

                num_topk = 5
                inf_topk = torch.topk(visual_hist, num_topk)
                sca_cans = visual_bins[inf_topk.indices] / 1000.0

                gc_a=GraphConstructorFor3DMatch()

                valid_mask_all = []
                for i in range(num_topk):
                    # perform not good
                    # if torch.abs(sca_cans[i]*1e3 - sca_avg) >= sca_sigma:
                    #     print("===> un-used ...")
                    #     continue

                    corr_3dm = sca_cans[i]*corr_3dm_raw
                    ...
                    # make graph from scales
                    alpha_sca = 0.0010 * 2 # varies with scenes 0.0010 0.0005
                    scale_mat = scales - sca_cans[i]
                    score_mat = torch.exp(-scale_mat**2 / (2 * alpha_sca * alpha_sca))
                    score_mat = score_mat*torch.matmul(score_mat,score_mat)
                    score_mat = score_mat.unsqueeze(0)
                    
                    corr_graph=gc_a(
                        inputs_c, 
                        config_fastmac_an["resolution"], 
                        config_fastmac_an["name"], 
                        config_fastmac_an["descriptor"], 
                        config_fastmac_an["inlier_thresh"])

                    corr_graph=corr_graph*score_mat
                    corr_graph=score_mat

                    degree_signal = torch.sum(corr_graph,dim=-1)
                    corr_laplacian = \
                        (torch.diag_embed(degree_signal)-corr_graph).squeeze(0)
                    
                    # note-0815 using the sysmetric laplacian matrix
                    # --- perform not good
                    # Dmat = torch.diag_embed(degree_signal).squeeze(0)
                    # Dmat_inv      = torch.linalg.inv(Dmat)
                    # Dmat_inv_half = torch.sqrt(Dmat_inv)
                    # # corr_laplacian = torch.matmul(Dmat_inv_half,corr_laplacian)
                    # # corr_laplacian = torch.matmul(corr_laplacian,Dmat_inv_half)
                    # corr_laplacian = torch.matmul(Dmat_inv,corr_laplacian)
                ...
                # note-0811
                # remove repeated idxs --- perform not good
                # valid_mask_all = list(set(valid_mask_all))
                valid_mask = valid_mask_all #set(valid_mask_all)

            '''
                scheme-7 our new try on turboreg
            '''
            is_used_scheme_7 = False
            is_used_scheme_7 = True
            if is_used_scheme_7:
                num_topk = 3
                inf_topk = torch.topk(visual_hist, num_topk)
                sca_cans = visual_bins[inf_topk.indices] / 1000.0

                valid_mask_all = torch.zeros_like(valid_mask).bool()
                for i in range(num_topk):
                    corr_3dm = sca_cans[i]*corr_3dm_raw
                    kpts_src = corr_3dm # [n,3]
                    kpts_dst = corr_3d  # [n,3]
                    # print("kpts_src: ", kpts_src)
                    ...
                    tmp = tmp.T
                    dis = torch.norm(kpts_dst-tmp, dim=1, keepdim=False)
                    valid_mask_cur = (dis <= 0.30) # 0.10
                    # summary
                    valid_mask_all = valid_mask_all | valid_mask_cur
