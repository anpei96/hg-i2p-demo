    ...
    def forward(self, data_dict):
        ...
        if not self.training:
            ...
            if is_used_scheme_6:
                ...
                # step 1. add scalar randomly or uniformly sampling
                basis = torch.arange(-10,11).cuda() * 0.01
                # basis = torch.arange(-1,2).cuda() * 0.05
                sca_samples_1k = sca_opt_1k + basis # [b]
                sca_samples_1k = sca_samples_1k.reshape((-1,1,1)) # [b,1,1]
                num_samples    = int(basis.size(0))
                # step 2. compute a series of graphs with sampled scalars
                #         using scalar map for graph construction
                scales_1k_tensor = scales_1k.repeat((num_samples,1,1)) # [b,n,n]
                sb_1k_tensor     = sca_samples_1k.repeat((1,param_n,param_n))
                # alpha_sca = 0.10 * 2 # used for scalar map
                alpha_sca = 10.0 * 2 # used for distance map (x 1k)
                score_mat = scales_1k_tensor - sb_1k_tensor # [b,n,n]
                
                '''
                    convert scalar map => distance map
                '''
                tmp_mat = w_colls_norm[:,:,0].unsqueeze(0)  # [1,n,n]
                tmp_mat = tmp_mat.repeat((num_samples,1,1)) # [b,n,n]
                score_mat = score_mat * tmp_mat
                # print("(pre) score_mat: ", score_mat[11])

                score_mat = \
                    torch.exp(-score_mat**2 / (2*alpha_sca*alpha_sca))
                # print("(aft) score_mat: ", score_mat[11])
                # score_mat[score_mat<0.950] = 0 # 0.950 used for scalar map
                score_mat[score_mat<0.950] = 0 # 0.950 used for distance map

                score_mat = \
                    score_mat*torch.einsum("bmn,bnk->bmk",score_mat,score_mat)
                # step 3. select best candidates via fast-mac
                valid_mask_all = []
                for b in range(num_samples):
                    degree_signal = torch.sum(score_mat[b],dim=-1)
                    corr_laplacian = \
                        (torch.diag_embed(degree_signal)-score_mat[b])
                    # print("corr_laplacian: ", corr_laplacian.size())
                    # print("degree_signal: ",  degree_signal.size())
                    corr_scores = graphFilter(
                ...
                # step 4. post-proc with turbo reg
                # kpts_src = corr_3dm # [n,3]
                # kpts_dst = corr_3d  # [n,3]
                # # run registration
                # trans = reger.run_reg(kpts_src, kpts_dst).cuda()
