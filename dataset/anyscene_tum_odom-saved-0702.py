...
class I2PHardPairDataset(Dataset):
    ...
    def __init__(
        ...
        self.map_pts = np.array(map_pcd.points)
        self.map_rgb = np.array(map_pcd.colors)
    ...
    def __getitem__(self, index, pose = np.eye(4)):
        data_dict = {}
        '''
        ...
        # get pose
        transform = data_info["transform_matrix"] #np.eye(4)
        # transform = np.eye(4)

        # read points with down-sampling
        '''
            note-0628
                generate I --- project from the map 
                               very noised due to pose error :(
                depth is smaller than 1.25 in scene_1, balance point cloud density and number
        '''
        is_disable_1 = True
        if is_disable_1:
            pixels, depths = render(self.map_pts, intrinsics, transform, True) #[h,w] [n,2]
            valid_map = (pixels[:,0]>=0) & (pixels[:,0]<data_dict["image_h"]) \
                & (pixels[:,1]>=0) & (pixels[:,1]<data_dict["image_w"]) \
                & (depths[:]>0)  & (depths[:]<1.25)
            points = self.map_pts[valid_map]
            points_rgb = self.map_rgb[valid_map]

        '''
            note-0628
                generate II --- project from the keyframe 
                depth is smaller than 1.25 in scene_1, balance point cloud density and number
            note-0629
                using the relative coordinate
            
            there is also a bug --- without using the current frame pose
        '''
        is_disable_2 = True
        if is_disable_2:
            keyframe_interval = 4 #10
            keyframe_id = int(index/keyframe_interval)*keyframe_interval # interval is 10
            data_info_key = self.tumparser.frames[keyframe_id]
            depth_key  = read_depth_image(data_info_key["depth_path"], depth_scale).astype(np.float)
            transform_key = data_info_key["transform_matrix"]
            depth_limit = 1.25
            points_mat = back_project(depth_key, intrinsics, depth_limit=depth_limit, return_matrix=True)
            valid_map = [points_mat[:,:,2] > 0]
            points_key = points_mat[valid_map] # [N,3]
            points_rgb_key = image[valid_map]

            transform_key_inv = np.linalg.inv(transform_key)
            # map_pts = apply_transform(points_key, transform_key_inv)
            
            # re-define the gt camera pose
            # transform = data_info["transform_matrix"] #np.eye(4)
            transform = np.matmul(transform, transform_key_inv)

            map_pts = points_key
            pixels, depths = render(map_pts, intrinsics, transform, True) #[h,w] [n,2]
            valid_map = (pixels[:,0]>=0) & (pixels[:,0]<data_dict["image_h"]) \
                & (pixels[:,1]>=0) & (pixels[:,1]<data_dict["image_w"]) \
                & (depths[:]>0)  & (depths[:]<2.25) # 1.25
            points = map_pts[valid_map]
            points_rgb = points_rgb_key[valid_map]
        ...
        '''
            note-0704
                generate IV --- project from the keyframe map 
                depth is smaller than 1.25 in scene_1, balance point cloud density and number
                using the relative coordinate
        '''        
        # keyframe back-projection
        keyframe_interval = 4 #10
        keyframe_id = int(index/keyframe_interval)*keyframe_interval # interval is 10
        data_info_key = self.tumparser.frames[keyframe_id]
        depth_key  = read_depth_image(data_info_key["depth_path"], depth_scale).astype(np.float)
        ...
        points_rgb_key = image[valid_map]
        transform_key_inv = np.linalg.inv(transform_key)
        points_key = apply_transform(points_key, transform_key_inv)

        pixels, depths = render(points_key, intrinsics, cur_pose, True) #[h,w] [n,2]
        valid_map = (pixels[:,0]>=0) & (pixels[:,0]<data_dict["image_h"]) \
            & (pixels[:,1]>=0) & (pixels[:,1]<data_dict["image_w"]) \
            & (depths[:]>0)  & (depths[:]<2.5)
        points = points_key[valid_map]
        points_rgb = points_rgb_key[valid_map]
