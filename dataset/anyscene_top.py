...
class I2PHardPairDataset(Dataset):
    ...
    def __init__(
        ...
        # tuned for correspondencing pruning
        # scene_spec_name = "fire" #"redkitchen"
        # is_use_scene_train = True
    ...
    def __getitem__(self, index: int):
        ...
        seg_path_a = _path[:-4] + "_sampd.png" # easy-level test o d
        ...
        if is_need_depth_v2:
            depth_path_v2 = _path[:-4] + "_depth_v2.bin"
        ...
        if self.return_corr_indices:
            ...
            '''
                note-0713 
                    check the metric of 2d-3d correspondences --- ok
            '''
            is_need_check_corr = False
            if is_need_check_corr:
                # step 1: randomly selected two correspondences --- ok
                coor_idx_1 = 666
                coor_idx_2 = 888-44 #777
                pix_1 = img_corr_pixels[coor_idx_1,:] # [v,u]
                pix_2 = img_corr_pixels[coor_idx_2,:] # [v,u]
                pts_1 = points[pcd_corr_indices[coor_idx_1],:] # [x,y,z]
                pts_2 = points[pcd_corr_indices[coor_idx_2],:] # [x,y,z]

                dep_path_v2 = _path[:-4] + "_depth_v2.bin"
                depth_bin = np.fromfile(dep_path_v2, np.int16)
                depth_bin = depth_bin.reshape((image.shape[0], image.shape[1]))
                zz_1   = depth[pix_1[0], pix_1[1]]/1000.0 # d
                zz_2   = depth[pix_2[0], pix_2[1]]/1000.0 # d
                z_1   = depth_bin[pix_1[0], pix_1[1]]/5000.0 # d
                z_2   = depth_bin[pix_2[0], pix_2[1]]/5000.0 # d

                tf_mat = transform
                kk_mat = intrinsics
                kk_mat_inv = np.linalg.inv(intrinsics)
                # exchange coordinates [v,u] => [u,v]
                pix_1c = np.ones((3,1))
                pix_1c[0,0] = pix_1[1]
                pix_1c[1,0] = pix_1[0]
                pix_2c = np.ones((3,1))
                ...
                # step 2: calculate vectors w and v
                w = pix_2c - z_1/z_2 * pix_1c
                w = np.matmul(kk_mat_inv, w)
                v = (pts_2 - pts_1).reshape((-1,1))
                wn = w/np.linalg.norm(w)
                vn = v/np.linalg.norm(v)
                print("wn: ", wn)
                print("vn: ", vn)
                rr_mat = tf_mat[0:3, 0:3]
                res = wn - np.matmul(rr_mat, vn)

                print("img_corr_pixels: ", img_corr_pixels.shape)
                print("transform: ", transform)
                print("pix_1: ", pix_1, " pix_2: ", pix_2)
                print("pts_1: ", pts_1, " pts_2: ", pts_2)
                print("z_1: ", z_1, " z_2: ", z_2, " ratio: ", z_1/z_2)
                print("zz_1: ", zz_1, " zz_2: ", zz_2, " ratio: ", zz_1/zz_2)
                print("res: ", res)
                assert 1==-1
